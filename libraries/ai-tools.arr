...
# use context alone doesn't chain transitively (foundations.arr's own
# "use context starter2024" isn't inherited a level down), so this file
# establishes starter2024 directly above. foundations.arr is pulled in via
# a qualified `import` (not `include`) and re-aliased below one name at a
# time -- `include`ing it here would re-trigger its "shadows a built-in"
# notices (translate/sin/cos/tan/range/dilate/image-url) as diagnostics
...
import url-file("https://raw.githubusercontent.com/bootstrapworld/starter-files/refs/heads/fall2026/libraries/", "foundations.arr") as Foundations
...
round-digits = Foundations.round-digits
image-entropy = Foundations.image-entropy
image-luminance = Foundations.image-luminance
image-symmetry-vertical = Foundations.image-symmetry-vertical
image-symmetry-horizontal = Foundations.image-symmetry-horizontal
dominant-rgb-colors = Foundations.dominant-rgb-colors
num-words = Foundations.num-words
num-sentences = Foundations.num-sentences
num-syllables = Foundations.num-syllables
sort-strings-ci = Foundations.sort-strings-ci
string-trim = Foundations.string-trim
easy-num-repr = Foundations.easy-num-repr
num-round-to = Foundations.num-round-to

####################################################################
# ai-tools.arr
#
# Everything AI: text/document feature engineering, similarity and
# recommendation tools, decision trees and clustering, n-gram
# language models, PCA, and neural networks.
#
# Depends on foundations.arr for image-analysis helpers (image-entropy,
# dominant-rgb-colors, etc.) used by decorate-image-table.
#
# Split out of core.arr, which used to hold everything directly.
# This is everything that used to live in the separate ai-library.arr.
####################################################################
...
fun classify(t :: Table, col, classifier) block:
  ...
  p-table = t.build-column(new-col, fn)
